    ...
    axios({
        ...
        if(response.status == 200) {
            dispatch({
                ...
                payload: {
                    ...historyCore, historyResource: response.data
                }
            })
    ...
    axios({
        ...
        if(response.status == 200) {
            dispatch({
                ...
                payload: {
                    ...historyCore, historyResourceDocu: response.data
                }
            })
        }
    }).catch(function (error) {
...
export const getAllHistoryUserCore = (user) => async(dispatch, getState) => {
    const { historyCore, sesion } = getState();
    const { TockenUser } = sesion;
    dispatch(setActiveSpinnerHistory(true));
    axios({
        ...
        if(response.status == 200) {
            dispatch({
                ...
                payload: {
                    ...historyCore, historyUserCore: response.data
                }
            })
            dispatch(getAllHistoryUserDocu(user?.id));
        }
    }).catch(function (error) {
        console.log(error);
        dispatch({
...
export const getAllHistoryUserDocu = (user) => async(dispatch, getState) => {
    const { historyCore, sesion } = getState();
    const { TockenUser } = sesion;
    axios({
        ...
        if(response.status == 200) {
            dispatch({
                ...
                payload: {
                    ...historyCore, historyUser: response.data
                }
            })
            dispatch(setActiveSpinnerHistory(false));
        }
